...
cat << "EOF"
		...
		This script will setup your Genesis L1 Testnet validator based on evmos v0.3 open codes.
		It is highly recomended to setup validator manually with all security protocols in mind.
		This script was made for demonstation purposes only.
...
cd ../config
sed -i 's/seeds = ""/seeds = "b1699e016721b193a08e24145700fed761fa5be6@65.21.121.202:26656"/' config.toml
sed -i 's/persistent_peers = "b1699e016721b193a08e24145700fed761fa5be6@65.21.121.202:26656"/' config.toml
rm -r genesis.json
...
echo Wait 1 hour for network sync
sleep 3600s
